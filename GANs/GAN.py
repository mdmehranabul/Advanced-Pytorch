...
x=16*(np.sin(theta)**3)
y=13*np.cos(theta) - 5*np.cos(2*theta) - 2*np.cos(3*theta) - np.cos(3*theta) - np.cos(4*theta)
sns.scatterplot(x=x,y=y)
#plt.show()

train_data=torch.Tensor(np.stack((x,y),axis=1))
train_labels=torch.zeros(TRAIN_DATA_COUNT)
train_set=[(train_data[i],train_labels[i]) for i in range(TRAIN_DATA_COUNT)]
...
for epoch in range(NUM_EPOCHS):
    for n,(real_samples,_) in enumerate(train_loader):
        ...
        if epoch%2==1:
            ...
            if epoch%11==0:
                print(epoch)
                print(f"Epoch: {epoch}, Disriminator Loss: {loss_discriminator}")
