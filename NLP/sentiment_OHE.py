...
print(f"X_train:{X_train.shape}, y_train: {y_train.shape}, X test:{X_test.shape}, y_test:{y_test.shape}")
# %%
...
for epoch in range(NUM_EPOCHS):
    ...
    for X_batch,y_batch in train_loader:
        optimizer.zero_grad()
        y_pred_lod=model(X_batch)
        loss=criterion(y_pred_lod,y_batch.long())
...
f"Accuracy of the model is {np.round(acc,3)*100}%"
# %%
